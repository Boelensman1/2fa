...
- `'v1 parameters produce the known hash'` — **unchanged**, and now explicitly
  marked as surviving until the v1 read path itself goes. (Amended 2026-09-17:
  that read path is gone, and the vector stayed anyway — it is now
  `'the sync parameters produce the known hash'`, because `createSyncKey`
  derives with exactly these numbers.)
...
- `'the v1 parameters are still reachable explicitly'` — new, because the
  migration depends on it. (Amended 2026-09-17: renamed to
  `'explicit parameters override the shipped default'` when the migration went.
  What it pins is unchanged and still load-bearing — a vault records its own
  parameters and must open under them, not under the shipped default.)
